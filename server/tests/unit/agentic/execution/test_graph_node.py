...
from agentic.decisions.schemas import AgentDecision, AgentUserInputRequest
...
@pytest.mark.asyncio
async def test_execute_step_writes_memory_update_on_need_input_decision() -> None:
    """
    NEED_INPUT fix: a clarifying-question decision is terminal-for-now
    (the agent is waiting on the user), same as FINAL -- it must
    produce a memory_updates artifact carrying the question as
    content, not be silently dropped the way it was before (only
    FINAL wrote to memory_updates, so AgentResponseMapper never ran
    for NEED_INPUT and execution_result.artifacts stayed empty).
    """
    ...
    handle.reasoning_context = ()
    handle.request = MagicMock()
    handle.request.context = build_agent_context(execution_id="exec-need-input")
    ...
    response = artifact["value"]
    assert response.agent_name == "legal"
    assert response.content == decision.user_input.question
    assert response.metadata["execution_id"] == "exec-need-input"
    assert response.metadata["termination_reason"] == TerminationReason.USER_INPUT_REQUIRED.value
