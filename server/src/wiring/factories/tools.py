...
a per-request `session: AsyncSession` and `allowed_library_ids`
...
method call) and read allowed_library_ids from
core.request_context at execute()-time. See tools/library/
library_lookup.py and tools/retrieval.py for where that actually
happens.
"""
